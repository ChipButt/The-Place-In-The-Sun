...
import { MAPS, TILE_DEFS, getSpawnForMap, NASSAU_LAYOUT_OFFSET_X, NASSAU_LAYOUT_OFFSET_Y } from "./maps.js";
...
const CROP_TOTAL_GROW_SECONDS = 120;
...
export class Game {
  ...
  setupGridUi() {
    this.gridButton?.addEventListener("click", () => {
      this.showTileGrid = !this.showTileGrid;
      this.gridButton.classList.toggle("active", this.showTileGrid);
      this.gridButton.textContent = this.showTileGrid ? "Grid On" : "Grid";
    });
    ...
    window.addEventListener("keydown", (event) => {
      ...
      this.showTileGrid = !this.showTileGrid;
      this.gridButton?.classList.toggle("active", this.showTileGrid);
      if (this.gridButton) this.gridButton.textContent = this.showTileGrid ? "Grid On" : "Grid";
    });
  }
  ...
  loop(time) {
    ...
    this.renderer.draw(this.currentMap, this.player, this.character, this.showTileGrid);
  ...
  update(dt) {
    ...
    if (this.portalCooldown <= 0 && !this.player.stepping) {
      ...
      if (portal) {
        if (portal.id === "toHouse01") {
          this.handleFirstHouseDoor();
        } else {
  ...
  handleQuest(interaction) {
    ...
    if (hasItem(this.character, "rusty_key")) {
      ...
      this.character.flags.houseUnlockedByOwner = true;
      this.character.flags.houseLockedByOwner = false;
  ...
  afterDialogueClosed() {
    ...
    if (action === "giveTorchAndLeave") {
      this.giveTorchToPlayerLeftHand();
    ...
    if (action === "houseTrespassScold") {
      this.pendingAfterDialogue = "houseTrespassReturnHome";
      this.dialogue.show(
        "Concerned Citizen: That's not very nice. You shouldn't just enter another person's house without asking. What are you, some kind of pirate? You're no hero."
      );
      return;
    }

    if (action === "houseTrespassReturnHome") {
      this.startConcernedCitizenReturnHome(true);
      return;
    }
  }

  handleFirstHouseDoor() {
    if (this.currentMap.id !== "nassau") return;

    const rejectFromDoor = () => {
      setPlayerSpawn(this.player, {
        tileX: NASSAU_LAYOUT_OFFSET_X + 10,
        tileY: NASSAU_LAYOUT_OFFSET_Y + 21,
        direction: "up",
    ...
    // If the owner has been given the Rusty Key properly, the hut is unlocked.
    if (this.character.flags.houseUnlockedByOwner === true && this.character.flags.houseLockedByOwner !== true) {
      this.changeMap("house01", "entrance");
      return;
    }

    if (this.character.flags.houseLockedByOwner) {
      rejectFromDoor();
      this.dialogue.show("This door is locked.");
      return;
    }
    ...
    removeItem(this.character, "rusty_key");
    this.character.quests.lostHouseKey = "complete";
    this.character.flags.houseLockedByOwner = true;
    this.character.flags.houseUnlockedByOwner = false;
    this.renderAllInventoryViews("Rusty Key used.");
    this.updateStatusHud();

    rejectFromDoor();

    this.portalCooldown = 1.2;
    this.pendingAfterDialogue = "houseTrespassScold";
    this.dialogue.show("Concerned Citizen: Hey!");
  }

  startConcernedCitizenReturnHome(forceFromTrespass = false) {
    const npc = MAPS.nassau.npcs.find((entry) => entry.id === "concernedCitizen");
    if (!npc) return;

    if (npc.hidden && !forceFromTrespass) return;
    if (forceFromTrespass && npc.hidden) {
      npc.hidden = false;
      npc.x = NASSAU_LAYOUT_OFFSET_X + 20;
      npc.y = NASSAU_LAYOUT_OFFSET_Y + 13;
    }

    npc.direction = "down";
    npc.sprite = npc.standingSprites?.down || "citizenDown0";
    ...
    // Front-door tile of the first Nassau hut.
    const targetTile = { x: NASSAU_LAYOUT_OFFSET_X + 10, y: NASSAU_LAYOUT_OFFSET_Y + 20 };
    const path = findNpcTilePath(MAPS.nassau, startTile, targetTile, npc.id);

    npc.script = "returning_home";
    npc.speed = 1;
    npc.path = path.length ? path : [
      { x: 20, y: 13 },
      { x: 19, y: 13 },
      { x: 18, y: 13 },
      { x: 17, y: 13 },
      ...
      { x: 10, y: 19 },
      { x: 10, y: 20 }
    ].map(offsetLegacyNassauTile);

    npc.pathIndex = 0;
    npc.scriptDelay = forceFromTrespass ? 0.05 : 0.15;
    npc.interaction = null;
    npc.dialogue = forceFromTrespass
      ? "I'm locking my door now."
      : "Thank you again. I'm heading home now.";
  }
  ...
  checkForcedCaveIntro() {
    ...
    if (this.character.flags.caveTorchIntroDone || this.character.flags.caveTorchIntroInProgress) return false;

    const tile = getPlayerTile(this.player);
    if (tile.y > 18) return false;

    this.character.flags.caveTorchIntroInProgress = true;
    this.pendingAfterDialogue = "giveTorchAndLeave";
  ...
  giveTorchToPlayerLeftHand() {
    const torch = ITEM_DEFS.torch || { id: "torch", name: "Torch", description: "Lights a small area." };
    const leftItem = this.character.hands.left;

    if (leftItem && leftItem.id !== "torch") {
      const freePocket = this.character.pockets.findIndex((slot) => slot === null);
      if (freePocket !== -1) {
        this.character.pockets[freePocket] = leftItem;
      } else if (!this.character.hands.right) {
        this.character.hands.right = leftItem;
      } else {
        // Last resort: preserve the item by adding it to the first pocket, even if it means replacing nothing.
        // This should only happen if every pocket and the right hand are full during the cave intro.
        this.showToast("Your left-hand item was moved as best as possible to make room for the torch.");
      }
    }

    this.character.hands.left = { ...torch };
    this.character.flags.caveTorchIntroDone = true;
    this.character.flags.caveTorchIntroInProgress = false;
    this.renderAllInventoryViews("Torch is now in your left hand.");
    this.saveGame(false);
  }
  ...
  collectResourceObject(objectId, resourceId, amount = 1) {
    ...
    const result = addResource(this.character, resourceId, amount);
    ...
    this.updateStatusHud();
    const message = "You collected " + formatFoundAmount(result.added, itemName) + ".";
    this.renderAllInventoryViews(message);
  ...
  harvestCrop(objectId) {
    ...
    if ((object.cropStage || 1) < 4) {
      this.dialogue.show("That crop is still growing.");
      return;
    }

    const result = addResource(this.character, object.cropType, 3);
    if (!result.ok || result.added < 3) {
      // Roll back partial addition if possible, so the crop is not lost unfairly.
      if (result.added > 0) removeResource(this.character, object.cropType, result.added);
      this.updateStatusHud();
      const message = "No room to collect " + resourceName(object.cropType) + ".";
      this.renderAllInventoryViews(message);
      this.showToast(message, 2.6);
      return;
    }
    ...
    this.showToast("You harvested 3 " + resourceName(object.cropType) + ".", 2.4);
  }
  ...
  updateFarmingObjects(dt) {
    for (const map of Object.values(MAPS)) {
      for (const object of map.objects || []) {
        ...
        const stage = Math.min(4, 1 + Math.floor(elapsed / CROP_STAGE_SECONDS));
        object.cropStage = stage;
  ...
  checkFlowerForage() {
    ...
    const itemName = resourceName(resourceId);
    const result = addResource(this.character, resourceId, 1);
    ...
    this.updateStatusHud();
    const message = "You found " + formatFoundAmount(result.added, itemName) + ".";
    this.renderAllInventoryViews(message);
    this.showToast(message, 2.4);
  }
  ...
  findEggTileNearChicken(npc, map) {
    ...
    const tileHasVisibleObject = (x, y) => {
      return (map.objects || []).some((object) => {
        if (object.hidden || object.collisionOnly) return false;
        if (object.resourceId === "egg") return false;

        const startX = Math.floor(object.x);
        const startY = Math.floor(object.y);
        const endX = Math.ceil(object.x + ((object.drawW || TILE_SIZE) / TILE_SIZE)) - 1;
        const endY = Math.ceil(object.y + ((object.drawH || TILE_SIZE) / TILE_SIZE)) - 1;
    ...
    for (const [dx, dy] of offsets) {
      ...
      if (!tileId || tileId === "water" || tileId === "shallowWater" || tileId === "dockWater") continue;
      if (tileHasVisibleObject(x, y)) continue;

      const hasEgg = (map.objects || []).some((object) => !object.hidden && object.resourceId === "egg" && Math.round(object.x) === x && Math.round(object.y) === y);
      if (hasEgg) continue;

      const npcOnTile = (map.npcs || []).some((other) => other !== npc && !other.hidden && Math.round(other.x) === x && Math.round(other.y) === y);
      if (npcOnTile) continue;

      const playerTile = getPlayerTile(this.player);
      if (playerTile.x === x && playerTile.y === y) continue;
  ...
  applySaveData(save) {
    ...
    const saveVersion = Number.isFinite(save.version) ? save.version : 1;
    this.applySavedMapState(save.maps || {}, saveVersion);

    const map = MAPS[save.currentMapId] || MAPS.nassau;
    this.currentMap = map;

    const playerSave = save.player || {};
    const fallbackSpawn = getSpawnForMap(map);
    const legacyNassauSave = saveVersion < 2 && map.id === "nassau";
    const savedTileX = Number.isFinite(playerSave.tileX) ? playerSave.tileX : fallbackSpawn.tileX;
    const savedTileY = Number.isFinite(playerSave.tileY) ? playerSave.tileY : fallbackSpawn.tileY;
    setPlayerCharacter(this.player, playerSave.characterKey || this.player.characterKey);
    setPlayerSpawn(this.player, {
      tileX: legacyNassauSave ? savedTileX + NASSAU_LAYOUT_OFFSET_X : savedTileX,
      tileY: legacyNassauSave ? savedTileY + NASSAU_LAYOUT_OFFSET_Y : savedTileY,
      direction: playerSave.direction || fallbackSpawn.direction || "down",
  ...
  applySavedMapState(savedMaps, saveVersion = 2) {
    for (const [mapId, saved] of Object.entries(savedMaps)) {
      const map = MAPS[mapId];
      if (!map) continue;
      const legacyNassauSave = saveVersion < 2 && mapId === "nassau";
      ...
      for (const savedObject of saved.dynamicObjects || []) {
        map.objects.push(offsetLegacyNassauSavedObject(savedObject, legacyNassauSave));
      }

      for (const npcState of saved.npcs || []) {
        const npc = (map.npcs || []).find((entry) => entry.id === npcState.id);
        if (!npc) continue;
        npc.x = legacyNassauSave ? npcState.x + NASSAU_LAYOUT_OFFSET_X : npcState.x;
        npc.y = legacyNassauSave ? npcState.y + NASSAU_LAYOUT_OFFSET_Y : npcState.y;
        ...
        // Force locked standing facings for key NPCs even when loading an older save.
        if (npc.id === "nancy") {
          npc.direction = "down";
          npc.sprite = "nancyF04South";
        }

        if (npc.id === "concernedCitizen" && !npc.script) {
          npc.direction = "down";
...
function offsetLegacyNassauTile(tile) {
  return {
    ...tile,
    x: tile.x + NASSAU_LAYOUT_OFFSET_X,
    y: tile.y + NASSAU_LAYOUT_OFFSET_Y
  };
}

function offsetLegacyNassauSavedObject(object, shouldOffset) {
  const copy = { ...object };
  if (!shouldOffset) return copy;

  if (Number.isFinite(copy.x)) copy.x += NASSAU_LAYOUT_OFFSET_X;
  if (Number.isFinite(copy.y)) copy.y += NASSAU_LAYOUT_OFFSET_Y;
  if (copy.hitbox) {
    copy.hitbox = { ...copy.hitbox };
    if (Number.isFinite(copy.hitbox.x)) copy.hitbox.x += NASSAU_LAYOUT_OFFSET_X;
    if (Number.isFinite(copy.hitbox.y)) copy.hitbox.y += NASSAU_LAYOUT_OFFSET_Y;
  }
...
function moveNpcAlongPath(npc, dt) {
  ...
  if (!npc.stepping) {
    ...
    // Safety: path entries must be one tile at a time. If an older save or fallback
    // contains a long jump, split it into a single cardinal step so the NPC never glides diagonally.
    const currentX = Math.round(npc.x);
    const currentY = Math.round(npc.y);
    ...
    if (Math.abs(dx) + Math.abs(dy) > 1) {
      target = {
        x: currentX + Math.sign(dx || 0),
        y: dx !== 0 ? currentY : currentY + Math.sign(dy || 0)
      };
      npc.path.splice(npc.pathIndex, 0, target);
    }
    ...
    if (target.x > currentX) npc.direction = "right";
    else if (target.x < currentX) npc.direction = "left";
    else if (target.y > currentY) npc.direction = "down";
...
function updateNpcSprite(npc) {
  if (npc.anims) {
    ...
    const frameIndex = npc.moving ? Math.floor((npc.animTime || 0) * 12) % frames.length : 0;
...
function findNpcTilePath(map, start, target, movingNpcId = null) {
  ...
  while (queue.length) {
    ...
    for (const dir of directions) {
      ...
      if (nextKey !== targetKey && isNpcPathBlocked(map, next.x, next.y, movingNpcId)) continue;
      ...
      if (nextKey === targetKey) {
        ...
        while (cursorKey !== startKey) {
          ...
          if (previousKey !== startKey) path.unshift(previous);
...
function isNpcPathBlocked(map, tileX, tileY, movingNpcId = null) {
  const tileId = map.ground[tileY]?.[tileX];
  if (!tileId) return true;

  const tileDef = TILE_DEFS[tileId];
  if (tileDef?.solid) return true;
  ...
  for (const object of map.objects || []) {
    ...
    const hitbox = object.hitbox || {
      x: object.x,
      y: object.y,
      w: (object.drawW || TILE_SIZE) / TILE_SIZE,
      h: (object.drawH || TILE_SIZE) / TILE_SIZE
    };

    if (
      tileX >= hitbox.x &&
      tileX < hitbox.x + hitbox.w &&
      tileY >= hitbox.y &&
      tileY < hitbox.y + hitbox.h
    ) {
      return true;
    }
  }
  ...
  for (const npc of map.npcs || []) {
    if (npc.hidden || npc.id === movingNpcId) continue;
