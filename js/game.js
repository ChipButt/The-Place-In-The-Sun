...
export class Game {
  ...
  handleFirstHouseDoor() {
    ...
    const rejectFromDoor = () => {
      // Do not leave the player standing on the portal tile. Otherwise the locked-door
      // dialogue immediately re-triggers and traps the player in a loop.
    ...
    if (this.character.quests.lostHouseKey === "complete" || this.character.flags.houseLockedByOwner) {
      rejectFromDoor();
    ...
    // Push the player back off the doorway so the owner can get in.
    rejectFromDoor();
  ...
  startConcernedCitizenReturnHome(forceFromTrespass = false) {
    ...
    if (forceFromTrespass && npc.hidden) {
      ...
      npc.y = 13;
      npc.direction = "left";
      npc.sprite = npc.standingSprites?.left || "citizenLeft0";
    }
