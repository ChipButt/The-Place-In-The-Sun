...
export class Game {
  ...
  checkForcedCaveIntro() {
    ...
    this.dialogue.show(
      "Lost Explorer: It's too dark in here without a torch. I've finally found my way out, and it took me ages to craft this one, but you definitely need it more than I do. Take it, and good luck."
    );
  ...
  renderLoot(message = "Tap an item to take it, or use Take All.") {
    ...
    if (available.length === 0) {
      this.lootSlots.innerHTML = "";
      this.lootMessage.textContent = "Empty.";
...
function getPlayerTile(player) {
  return {
    x: Math.round((player.x / 48) - 0.5),
    y: Math.round((player.y / 48) - 0.5)
  };
}
