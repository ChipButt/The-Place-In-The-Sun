...
export function findInteraction(map, player) {
  ...
  for (const npc of map.npcs || []) {
    if (rectsOverlap(reachBox, inflate(getNpcHitbox(npc), 14))) {
  ...
  for (const object of map.objects || []) {
    ...
    if (rectsOverlap(reachBox, inflate(getObjectHitbox(object), 16))) {
...
export function findInteractionAtWorldPoint(map, worldX, worldY) {
  ...
  for (const npc of map.npcs || []) {
    if (rectsOverlap(pointBox, inflate(getNpcHitbox(npc), 18))) {
      return { type: "dialogue", text: `${npc.name}: ${npc.dialogue}` };
    }
  }
  ...
  for (const object of [...(map.objects || [])].reverse()) {
    if (!object.interaction) continue;
    const tappable = object.tapbox ? getTileRect(object.tapbox) : inflate(getObjectHitbox(object), 18);
