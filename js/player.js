...
import { DEFAULT_CHARACTER } from "./assets.js";
...
export function createPlayer(spawn) {
  ...
  return {
    ...
    characterKey: point.characterKey || DEFAULT_CHARACTER
  };
}
...
export function setPlayerSpawn(player, spawn) {
  ...
  player.animTime = 0;
  player.characterKey = point.characterKey || player.characterKey || DEFAULT_CHARACTER;
}
...
function normaliseSpawn(spawn) {
  if (Number.isFinite(spawn.tileX) && Number.isFinite(spawn.tileY)) {
    return {
      ...
      characterKey: spawn.characterKey
    };
  }
...
export function setPlayerCharacter(player, characterKey) {
  player.characterKey = characterKey || DEFAULT_CHARACTER;
  player.animTime = 0;
}
