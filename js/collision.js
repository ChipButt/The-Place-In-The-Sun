...
export function canMoveTo(map, player, nextX, nextY) {
  if (crossesBlockedEdge(map, player, nextX, nextY)) return false;

  const box = getPlayerHitbox(player, nextX, nextY);
  return !hitsSolidTile(map, box) && !hitsSolidObject(map, box);
}

function crossesBlockedEdge(map, player, nextX, nextY) {
  const edgeBlocks = map.edgeBlocks || [];
  if (!edgeBlocks.length) return false;

  const from = getTileFromWorld(player.x, player.y);
  const to = getTileFromWorld(nextX, nextY);
  ...
  return edgeBlocks.some((edge) => {
    return (
      (edge.x === from.x && edge.y === from.y && edge.direction === direction) ||
      (edge.x === to.x && edge.y === to.y && edge.direction === opposite)
    );
  });
}
...
function getTileFromWorld(x, y) {
  return {
    x: Math.round((x / TILE_SIZE) - 0.5),
    y: Math.round((y / TILE_SIZE) - 0.5)
  };
}
...
export function getNpcHitbox(npc) {
  if (npc.hitbox) {
    return {
      x: npc.hitbox.x * TILE_SIZE,
      y: npc.hitbox.y * TILE_SIZE,
      w: npc.hitbox.w * TILE_SIZE,
      h: npc.hitbox.h * TILE_SIZE
    };
  }

  const w = Math.min(TILE_SIZE, npc.drawW || TILE_SIZE);
  const h = Math.min(TILE_SIZE, npc.drawH || TILE_SIZE);

  return {
    x: (npc.x * TILE_SIZE) + ((TILE_SIZE - w) / 2),
    y: (npc.y * TILE_SIZE) + (TILE_SIZE - h),
    w,
    h
  };
}
