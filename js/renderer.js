...
export function createRenderer(canvas, images) {
  ...
  function drawHouseObject(object) {
    ...
    // Built from the new basictiles sprite sheet so the hut is no longer an old standalone PNG.
    for (let row = 0; row < 3; row++) {
      for (let col = 0; col < 4; col++) {
        drawSprite("houseWall", x + col * tile, y + row * tile, tile, tile);
      }
    }

    drawSprite("houseWindow", x + tile * 0.35, y + tile * 0.4, tile, tile);
    drawSprite("houseWindow", x + tile * 2.65, y + tile * 0.4, tile, tile);
    drawSprite("houseDoor", x + tile * 1.45, y + tile * 1.8, tile * 1.1, tile * 1.2);

    ctx.save();
    ctx.fillStyle = "rgba(62, 38, 16, 0.95)";
    ctx.fillRect(Math.round(x - 6), Math.round(y - 18), Math.round(tile * 4 + 12), 22);
    ctx.fillStyle = "rgba(91, 55, 22, 0.92)";
    ctx.fillRect(Math.round(x - 2), Math.round(y - 8), Math.round(tile * 4 + 4), 15);
    ctx.restore();
  }
