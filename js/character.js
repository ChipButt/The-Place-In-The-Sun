...
export function addResource(character, resourceId, amount = 1) {
  ...
  return getResourceCount(character, resourceId);
}
...
export function addStackableItemToInventory(character, itemId, amount = 1) {
  ...
  for (const slot of character.pockets) {
    ...
    remaining -= add;
    if (remaining <= 0) return { ok: true, item: { ...def, qty: amount }, reason: `Added ${amount} ${def.name}.` };
  }
  while (remaining > 0) {
    const freeIndex = character.pockets.findIndex((slot) => slot === null);
    if (freeIndex === -1) return { ok: false, reason: "Your pockets are full." };
    const add = Math.min(maxStack, remaining);
    character.pockets[freeIndex] = { ...def, qty: add };
    remaining -= add;
  }
  return { ok: true, item: { ...def, qty: amount }, reason: `Added ${amount} ${def.name}.` };
}
